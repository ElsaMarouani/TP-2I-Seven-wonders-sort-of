...
class World {
  constructor() {
    ...
    this.init();
    setTimeout(
      // Le setTimeout permet de fixer la fin du jeu à défaut d'avoir pu faire mieux
      () => process.abort(),
      100000
    );
  }
  ...
  buildingWonders() {
    for (let city = 0; city < this.listCities_.length; city++) {
      ...
      for (
        let wonder = 0;
        wonder < this.listWondersAchieved_.length;
        wonder++
      ) {
  ...
  furyOfTheGods(city) {
    for (let wonderD = 0; wonderD < city.listWonders_.length; wonderD++) {
      // Pour toutes les merveilles de cette ville
      if (city.listWonders_[wonderD].isFinished_ === 1) {
        // Si la merveille est finie
...
*/
    ...
    console.log(
      '|   Olympia    /                          \\ | A B | Halicarnassus                   _________                                                 |                          ',
  ...
  showWorld() {
    ...
    for (let cityW = 0; cityW < this.listCities_.length; cityW++) {
      ...
      this.listCities_[cityW].soulForTheArmy(
        this.listCities_[cityW].gold_ * this.rand(0.2, 0.5)
      ); // Investit de20 à 50% de son or dans l'armée
      this.listCities_[cityW].scienceInvest(
        this.listCities_[cityW].gold_ * this.rand(0.1, 0.3)
      ); // Investit de 10 à 30% de son or dans la recherche
    }
